...
class FinaceListController {
  ...
  Future<bool> validateInput(
      int? id, int? idFinance, BuildContext context) async {
    if (financaControlller.currentState!.validate()) {
      id != null
          ? await updateFinance(id, context)
          : await insertFinance(context, idFinance);
  ...
  Future<void> insertFinance(BuildContext context, int? idFinance) async {
    try {
      ...
      /* if (idFinance != null) {
        await dataBaseFinanceRepository.updateDateFinanceById(
            Format.formatDate(Format.dateParse(textControllerDataFinanca.text)), false
            idFinance);
      }*/
    } catch (e) {
      log(e.toString());
